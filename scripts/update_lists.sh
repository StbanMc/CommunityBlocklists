...
merge_ut1() {
    ...
    if curl -sfL "$url" -o "$tmp" 2>/dev/null; then
        local b=$(wc -l < "$BL_DIR/$l/domains" 2>/dev/null || echo 0)
        { cat "$BL_DIR/$l/domains" 2>/dev/null || true; cat "$tmp"; } \
            | grep -v "^#" | grep -v "^$" | tr "[:upper:]" "[:lower:]" | sort -u > "$TMP_DIR/m"
        mv "$TMP_DIR/m" "$BL_DIR/$l/domains"
        local a=$(wc -l < "$BL_DIR/$l/domains")
        local d=$((a - b)); [ "$d" -gt 0 ] && echo "    +${d} (${b}->${a})"
    else echo "    (skip)"; fi
}
...
merge_list() {
    ...
    if curl -sfL "$url" -o "$tmp" 2>/dev/null; then
        ...
        } | tr "[:upper:]" "[:lower:]" | tr -d "\r" | grep -E "^[a-z0-9]" | sort -u > "$TMP_DIR/m"
        mv "$TMP_DIR/m" "$BL_DIR/$l/domains"
        local a=$(wc -l < "$BL_DIR/$l/domains")
        local d=$((a - b)); [ "$d" -gt 0 ] && echo "    +${d} (${b}->${a})"
    else echo "    (skip)"; fi
...
merge_ut1 ads adv; merge_ut1 adult porn; merge_ut1 aggressive aggressive
merge_ut1 audio-video webtv; merge_ut1 bank finance/banking; merge_ut1 bitcoin finance/trading
merge_ut1 chat chat; merge_ut1 cryptojacking spyware; merge_ut1 dangerous_material violence
merge_ut1 dating dating; merge_ut1 download downloads; merge_ut1 drugs tracker/drugs
merge_ut1 dynamic-dns dynamic; merge_ut1 fakenews news; merge_ut1 filehosting imagehosting
merge_ut1 financial finance/other; merge_ut1 forums forum; merge_ut1 gambling gamble
merge_ut1 games hobby/games-online; merge_ut1 hacking hacking; merge_ut1 jobsearch jobsearch
merge_ut1 lingerie sex/lingerie; merge_ut1 malware spyware; merge_ut1 phishing hacking
merge_ut1 porn porn; merge_ut1 press news; merge_ut1 proxy anonvpn
merge_ut1 radio webradio; merge_ut1 redirector redirector; merge_ut1 remote-control remotecontrol
merge_ut1 sexual_education sex/education; merge_ut1 shopping shopping
merge_ut1 shortener urlshortener; merge_ut1 social_networks socialnet
merge_ut1 sports recreation/sports; merge_ut1 stalkerware spyware; merge_ut1 vpn anonvpn
merge_ut1 warez warez; merge_ut1 violence violence; merge_ut1 webmail webmail
...
find "$BL_DIR" -name "domains" -exec dirname {} \; | while read -r d; do
    [ ! -f "$d/urls" ] && touch "$d/urls"; done
echo "Done."
